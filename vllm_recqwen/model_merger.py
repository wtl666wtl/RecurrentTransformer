...
def test_fsdp_state_dict(
    ...
    original_model = auto_model_class.from_pretrained(original_hf_model_path, trust_remote_code=True, torch_dtype=torch.bfloat16)
...
def patch_model_generation_config(model, hf_model_path):
    ...
    if model.can_generate():
        try:
            model.generation_config = GenerationConfig.from_pretrained(hf_model_path, trust_remote_code=True)
        except OSError:
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    with torch.device("meta"):
        model = auto_model.from_config(config, trust_remote_code=True, torch_dtype=torch.bfloat16)
